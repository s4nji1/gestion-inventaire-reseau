    ...
    <div class="w-full">
        <div class="flex flex-col md:flex-row md:items-center md:justify-between mb-6">
            <div>
                ...
                <p class="mt-1 text-gray-600">Manage your network equipment inventory.</p>
            </div>
            ...
            <div class="p-4">
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    <div class="md:col-span-1">
                        <form action="{{ route('equipment.search') }}" method="GET" class="flex">
                            <input type="text" name="query" value="{{ $query ?? '' }}"
                                placeholder="Search equipment..."
                                class="flex-1 shadow-sm border-gray-300 rounded-l-md focus:border-blue-300 focus:ring focus:ring-blue-200 focus:ring-opacity-50">
                            <button type="submit"
                    ...
                    <div class="md:col-span-2">
                        <form action="{{ route('equipment.filter') }}" method="GET"
                            class="grid grid-cols-1 md:grid-cols-3 gap-4">
                            <div>
                                <label for="category_id" class="block text-sm font-medium text-gray-700">Category</label>
                                <select id="category_id" name="category_id"
                                    class="mt-1 block w-full shadow-sm border-gray-300 rounded-md focus:border-blue-300 focus:ring focus:ring-blue-200 focus:ring-opacity-50">
                            ...
                            <div class="flex items-end">
                                ...
                                @if (isset($categoryId) || isset($statusId) || isset($query))
        ...
        <div class="bg-white rounded-lg shadow">
            <div class="px-4 py-3 border-b border-gray-200 flex items-center justify-between">
                <h4 class="font-semibold text-gray-800">Equipment List</h4>
                            ...
                            <tr>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full"
                                        style="background-color: {{ $item->status->color }}25; color: {{ $item->status->color }}">
                                        {{ $item->status->name }}
                                    </span>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                    <div class="flex justify-end space-x-2">
                                        ...
                                        <a href="{{ route('movement.history', $item) }}"
                                            class="text-green-600 hover:text-green-900" title="View Movement History">
                                            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none"
                                                viewBox="0 0 24 24" stroke="currentColor">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            ...
                            <tr>
                                <td colspan="6" class="px-6 py-10 text-center text-gray-500">
                                    <div class="flex flex-col items-center">
                                        ...
                                        @if (isset($categoryId) || isset($statusId) || isset($query))
                                            <a href="{{ route('equipment.index') }}"
                                                class="mt-2 text-blue-600 hover:text-blue-800">Clear filters and try
                                                again</a>
                                        @else
                                            <a href="{{ route('equipment.create') }}"
                                                class="mt-2 text-blue-600 hover:text-blue-800">Add your first equipment</a>
                                        @endif
                                    </div>
                                </td>
                            </tr>
            ...
            <div class="flex space-x-2">
                <!-- Simple CSV Export -->
                <a href="{{ route('equipment.export.csv') }}"
                    class="inline-flex items-center px-4 py-2 bg-green-600 border border-transparent rounded-md font-semibold text-xs text-blue uppercase tracking-widest hover:bg-green-500">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 mr-2" fill="none" viewBox="0 0 24 24"
                        stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                ...
                <!-- Filtered CSV Export -->
                <a href="{{ route('equipment.export.filtered-csv') }}?category_id={{ request('category_id') }}&status_id={{ request('status_id') }}"
                    class="inline-flex items-center px-4 py-2 bg-blue-600 border border-transparent rounded-md font-semibold text-xs text-blue uppercase tracking-widest hover:bg-blue-500">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 mr-2" fill="none" viewBox="0 0 24 24"
                        stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            d="M3 4a1 1 0 011-1h16a1 1 0 011 1v2.586a1 1 0 01-.293.707l-6.414 6.414a1 1 0 00-.293.707V17l-4 4v-6.586a1 1 0 00-.293-.707L3.293 7.293A1 1 0 013 6.586V4z" />
