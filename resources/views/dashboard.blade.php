    ...
    <div class="w-full px-4">
        ...
        <div class="bg-white rounded-lg shadow mb-6 overflow-hidden">
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 divide-y lg:divide-y-0 lg:divide-x divide-gray-200">
                ...
                <!-- Categories Card + Dropdown inside an Alpine component -->
                <div x-data="{ open: false }">
                    <!-- Category Count Card (clickable) -->
                    <div @click="open = !open"
                        class="p-6 flex items-center cursor-pointer bg-white rounded-lg shadow hover:bg-gray-100 transition">
                    ...
                    <!-- Dropdown of categories (show/hide) -->
                    <div x-show="open" x-transition
                        class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6 mt-6">
                        @foreach ($categoryEquipmentCounts as $category)
                            <div
                                class="bg-white shadow rounded-lg p-6 flex items-center space-x-4 transform transition-transform duration-300 hover:translate-x-2">
                                <!-- Icon Placeholder -->
                                ...
                                <div>
                                    ...
                                    <p class="text-sm text-gray-500">{{ $category->equipment_count }} Equipment</p>
                                </div>
                            </div>
                        @endforeach
                    </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
            ...
            <div class="bg-white rounded-lg shadow">
                <div class="px-4 py-3 border-b border-gray-200 flex items-center justify-between">
                    ...
                    <div class="flex space-x-2">
                        <button id="statusChartTypeBar"
                            class="px-2 py-1 bg-blue-100 text-blue-600 rounded text-xs font-medium hover:bg-blue-200">Bar</button>
                        <button id="statusChartTypePie"
                            class="px-2 py-1 bg-gray-100 text-gray-600 rounded text-xs font-medium hover:bg-blue-100">Pie</button>
                    </div>
                </div>
                <div class="p-4">
            ...
            <div class="bg-white rounded-lg shadow">
                <div class="px-4 py-3 border-b border-gray-200 flex items-center justify-between">
                    ...
                    <div class="flex space-x-2">
                        <button id="categoryChartTypeBar"
                            class="px-2 py-1 bg-gray-100 text-gray-600 rounded text-xs font-medium hover:bg-blue-100">Bar</button>
                        <button id="categoryChartTypePie"
                            class="px-2 py-1 bg-blue-100 text-blue-600 rounded text-xs font-medium hover:bg-blue-200">Pie</button>
                    </div>
                </div>
                <div class="p-4">
            ...
            <div>
                <div class="bg-white rounded-lg shadow">
                    <div class="px-4 py-3 border-b border-gray-200 flex items-center justify-between">
                        ...
                        <a href="{{ route('maintenance.index') }}" class="text-sm text-blue-600 hover:underline">View
                            All</a>
                    </div>
                        ...
                        <div class="overflow-x-auto">
                            <table class="w-full text-center bg-white border border-gray-200">
                                <thead class="bg-gray-50">
                                    <tr>
                                        <th
                                            class="px-4 py-2 border-b text-center text-xs font-medium text-gray-500 uppercase tracking-wider">
                                            Equipment</th>
                                        <th
                                            class="px-4 py-2 border-b text-center text-xs font-medium text-gray-500 uppercase tracking-wider">
                                            Type</th>
                                    ...
                                    @forelse ($latestMaintenanceRecords ?? [] as $record)
                                        <tr class="hover:bg-gray-50">
                                            <td class="px-4 py-2 text-sm text-gray-800 text-center">
                                                {{ $record->equipment->name }}</td>
                                            <td class="px-4 py-2 text-sm text-gray-800 text-center">
                                                {{ $record->maintenance_type }}</td>
                                            <td class="px-4 py-2 text-sm text-center">
                                                @switch($record->status)
                                                    ...
                                                    @case('in_progress')
                                                        <span class="px-2 py-1 text-xs rounded-full bg-blue-100 text-blue-800">In
                                                            Progress</span>
                                                    @break
                                            ...
                                            <td class="px-4 py-2 text-sm text-gray-800 text-center">
                                                {{ $record->start_date->format('M d, Y') }}</td>
                                        </tr>
                                        @empty
                                            <tr>
                                                <td colspan="4"
                                                    class="px-4 py-3 text-center text-sm text-gray-500 text-center">No
                                ...
                                <table class="w-full text-center bg-white border border-gray-200">
                                    ...
                                    <tbody class="divide-y divide-gray-200">
                                        @forelse ($latestMovements ?? [] as $movement)
                                            <tr class="hover:bg-gray-50">
                                                <td class="px-4 py-2 text-sm text-gray-800 text-center">
                                                    {{ $movement->equipment->name }}</td>
                                                <td class="px-4 py-2 text-sm text-center">
                                                    @switch($movement->type)
                                                        @case('entry')
                                                            <span
                                                                class="px-2 py-1 text-xs rounded-full bg-green-100 text-green-800">Entry</span>
                                                        @break

                                                        @case('exit')
                                                            <span
                                                                class="px-2 py-1 text-xs rounded-full bg-red-100 text-red-800">Exit</span>
                                                        @break
                                                ...
                                                <td class="px-4 py-2 text-sm text-center">
                                                    @if ($movement->from_status_id)
                                                        <span
                                                            class="text-sm font-medium">{{ $movement->fromStatus->name }}</span>
                                                    @else
                                                ...
                                                <td class="px-4 py-2 text-sm text-gray-800 text-center">
                                                    {{ $movement->created_at->format('M d, Y') }}</td>
                                            </tr>
                                            @empty
                                                <tr>
                                                    <td colspan="4"
                ...
                document.addEventListener('DOMContentLoaded', function() {
                    ...
                    if (statusChartElement) {
                        ...
                        document.getElementById('statusChartTypeBar').addEventListener('click', function() {
                            ...
                            document.getElementById('statusChartTypePie').classList.remove('bg-blue-100',
                                'text-blue-600');
                            document.getElementById('statusChartTypePie').classList.add('bg-gray-100',
                                'text-gray-600');
                        });
                        ...
                        document.getElementById('statusChartTypePie').addEventListener('click', function() {
                            ...
                            this.classList.remove('bg-gray-100', 'text-gray-600');
                            this.classList.add('bg-blue-100', 'text-blue-600');
                            document.getElementById('statusChartTypeBar').classList.remove('bg-blue-100',
                                'text-blue-600');
                            document.getElementById('statusChartTypeBar').classList.add('bg-gray-100',
                                'text-gray-600');
                        });
                    }
                    ...
                    if (categoryChartElement) {
                        ...
                        document.getElementById('categoryChartTypePie').addEventListener('click', function() {
                            ...
                            this.classList.remove('bg-gray-100', 'text-gray-600');
                            this.classList.add('bg-blue-100', 'text-blue-600');
                            document.getElementById('categoryChartTypeBar').classList.remove('bg-blue-100',
                                'text-blue-600');
                            document.getElementById('categoryChartTypeBar').classList.add('bg-gray-100',
                                'text-gray-600');
                        });
                        ...
                        document.getElementById('categoryChartTypeBar').addEventListener('click', function() {
                            ...
                            this.classList.remove('bg-gray-100', 'text-gray-600');
                            this.classList.add('bg-blue-100', 'text-blue-600');
                            document.getElementById('categoryChartTypePie').classList.remove('bg-blue-100',
                                'text-blue-600');
                            document.getElementById('categoryChartTypePie').classList.add('bg-gray-100',
                                'text-gray-600');
                        });
                    }
                });
